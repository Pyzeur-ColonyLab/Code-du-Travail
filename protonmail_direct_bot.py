#!/usr/bin/env python3
"""
ProtonMail Direct Email Bot for Code du Travail - Mistral 7B Fine-tuned Model

This bot connects directly to ProtonMail servers (no Bridge required)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/protonmail_direct_bot.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class ProtonMailDirectBot:
    def __init__(self):
        self.model = None
        self.tokenizer = None
        ...
        # ProtonMail Direct configuration (no Bridge)
        self.email_address = os.getenv('PROTONMAIL_ADDRESS')
        self.email_password = os.getenv('PROTONMAIL_PASSWORD')
        
        # ProtonMail Plus servers
        self.imap_host = "mail.protonmail.ch"
        self.imap_port = 993
        self.smtp_host = "mail.protonmail.ch" 
        self.smtp_port = 587
        ...
        logger.info(f"ProtonMail Direct bot initialized for: {self.email_address}")
        logger.info(f"Device: {self.device}")
        logger.info(f"IMAP: {self.imap_host}:{self.imap_port} (SSL)")
        logger.info(f"SMTP: {self.smtp_host}:{self.smtp_port} (STARTTLS)")
...
Question: {question} [/INST]"""
    ...
    def connect_imap(self):
        """Connect to ProtonMail IMAP directly"""
        try:
            # Create SSL context
    ...
    def send_email(self, to_email: str, subject: str, body: str):
        """Send email response via ProtonMail SMTP directly"""
        try:
            msg = MIMEMultipart()
    ...
    def start(self):
        """Start the ProtonMail Direct bot"""
        logger.info("Starting ProtonMail Direct Bot for Code du Travail...")
        ...
        # Test connections
        logger.info("Testing ProtonMail direct connections...")
        try:
            mail = self.connect_imap()
            mail.logout()
        ...
        logger.info("ProtonMail Direct bot ready! Starting email monitoring...")
...
def main():
    ...
    try:
        ...
        bot = ProtonMailDirectBot()
        bot.start()
        
    except KeyboardInterrupt:
        logger.info("ProtonMail Direct bot stopped by user")
    except Exception as e:
        logger.error(f"ProtonMail Direct bot error: {e}")
        raise
